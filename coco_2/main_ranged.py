...
from networks import MiddleActionNetwork, LowActionNetwork_simple, LowActionNetwork_complex, LowActionNetwork_simplest
...
state_file_path = '../data/ranged/ranged_state.csv'
pos_file_path = '../data/ranged/ranged_pos.csv'
middleaction_file_path = '../data/ranged/ranged_middle_action.csv'
lowaction_file_path = '../data/ranged/ranged_low_action.csv'
...
if (train_low_action_model == True):
    ...
    def trainer_build(model, optimizer, train_loader, test_loader, network):

        LowActionTrainer = LowActionModelTrainer(model, train_loader, test_loader, optimizer, dim,
                                                 channels, low_action_num, y=True)
        LowActionTrainer.train(epoch_num, network, epoch_record=1)


    def simplest_model_build():
        ...
        return LowAction_model, action_optimizer


    def simple_model_build():
    ...
    model, optimizer = simplest_model_build()
    trainer_build(model, optimizer, train_loader, test_loader, network='1')
    print('end')

    train_loader, test_loader = data_load(1, 1, [False, True, False])
    model, optimizer = simple_model_build()
    trainer_build(model, optimizer, train_loader, test_loader, network='2')
    print('end')

    train_loader, test_loader = data_load(64, 64, [False, False, True])
    model, optimizer = complex_model_build()
    trainer_build(model, optimizer, train_loader, test_loader, network='3')
    print('end')
